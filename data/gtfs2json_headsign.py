...
START_TIME = get_seconds('05:00:00')
...
# 目的地名を取得
headsign_names = trips_df["trip_headsign"].unique().tolist()

for headsign_name in headsign_names:
    headsign_trips_df = trips_df[trips_df["trip_headsign"] == headsign_name]
    # 平日のtrip_idを取得
    trip_ids = headsign_trips_df[headsign_trips_df["service_id"].isin(day_service_id)]["trip_id"].unique().tolist()
    ...
    for i, trip_id in enumerate(trip_ids):
        ...
        trip_data["vendor"] = 0
        # lat, lonを抽出
        shape_id = headsign_trips_df[headsign_trips_df["trip_id"] == trip_id]["shape_id"].values[0]
    ...
    with open('./src/json_data/' + headsign_name + '.json', 'w') as f:
